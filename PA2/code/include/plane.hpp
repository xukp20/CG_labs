...
public:
    ...
    bool intersect(const Ray &r, Hit &h, float tmin) override {
        ...
        float n_dot_rd = Vector3f::dot(norm, r.getDirection());
        if (fabs(n_dot_rd) < 1e-6) {
            return false;
        }

        float t = (d - Vector3f::dot(norm, r.getOrigin())) / n_dot_rd;
        if (t < tmin || t <= 0) {
            return false;
        }

        if (t <= h.getT()) {
            Vector3f normal = Vector3f::dot(norm, r.getDirection()) < 0 ? norm : -norm;
